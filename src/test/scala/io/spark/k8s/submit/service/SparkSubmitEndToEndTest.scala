...
class SparkSubmitEndToEndTest extends AnyFlatSpec
  ...
  private val NoTemplateApp = "no-template-app"
  private val BothTemplatesApp = "both-templates-app"
  private val DriverOnlyApp = "driver-only-app"
  private val ExecutorOnlyApp = "executor-only-app"
  private val ConcurrentApp1 = "concurrent-app-1"
  private val ConcurrentApp2 = "concurrent-app-2"
  private val NamespaceTestApp = "namespace-test-app"
  private val CleanupTestApp = "cleanup-test-app"
  private val CustomPodNameApp = "custom-pod-name-app"
  private val CustomAppIdApp = "custom-app-id-app"
  private val AutoGeneratedIdsApp = "auto-generated-ids-app"
  ...
  private def createSubmitter(): SparkSubmitter = {
    ...
    field.set(provider, getKubeClient)
  ...
  it should "submit Spark job WITHOUT pod templates" in {
    ...
    val response = submitter.submitJob(SparkSubmitRequest(args, null, null))
  ...
  it should "submit Spark job WITH driver and executor pod templates" in {
    ...
    if (podLabels.containsKey("custom-label")) {
      podLabels.get("custom-label") shouldBe "driver-template"
    }
  }
  ...
  it should "submit Spark job WITH driver template ONLY" in {
    ...
    if (podLabels.containsKey(DriverOnlyLabel)) {
      podLabels.get(DriverOnlyLabel) shouldBe "true"
    }
  }
  ...
  it should "handle CONCURRENT Spark application submissions with resource isolation" in {
    ...
    val response1 = submitter.submitJob(SparkSubmitRequest(args1, null, null))
    val response2 = submitter.submitJob(SparkSubmitRequest(args2, null, null))
  ...
  it should "verify resources created by our code (ConfigMap volume, SPARK_CONF_DIR, namespace key)" in {
    val submitter = createSubmitter()
    val masterUrl = getKubeClient.getConfiguration.getMasterUrl
    val args = buildSparkSubmitArgs(masterUrl, SparkPiClass, "verify-our-code-app",
      getTestNamespace, SparkImage, LocalJar)

    val response = submitter.submitJob(SparkSubmitRequest(args, null, null))

    response should not be null
    response.appName shouldBe "verify-our-code-app"
  ...
  it should "set owner references for garbage collection (ConfigMap and Service reference driver Pod)" in {
    val submitter = createSubmitter()
    val masterUrl = getKubeClient.getConfiguration.getMasterUrl
    val args = buildSparkSubmitArgs(masterUrl, SparkPiClass, "owner-ref-test-app",
      getTestNamespace, SparkImage, LocalJar)

    val response = submitter.submitJob(SparkSubmitRequest(args, null, null))

    response should not be null
    response.appName shouldBe "owner-ref-test-app"
    ...
    configMapOwnerRefs.get(0).getKind shouldBe "Pod"
    ...
    serviceOwnerRefs.get(0).getKind shouldBe "Pod"
